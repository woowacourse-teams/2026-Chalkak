...
출력은 항상 손실 WebP로 다시 인코딩한다(`POST_WEBP_QUALITY`). 무손실로 올린 이미지도
손실로 바뀌고, 이미 손실 압축된 이미지는 세대 손실이 한 번 더 누적된다. EXIF를 제거하고
방향을 픽셀에 반영하려면 어차피 다시 인코딩해야 하므로 감수하는 트레이드오프다.
...
- 업로드 URL 발급 실패는 삭제를 뜻하는 404를 제외하고 상태 코드와 관계없이 재시도한다. 이 요청을
  영구 콜백 실패로 소비하면 staging과 처리 상태가 닫히지 않은 채 남기 때문이다.
- 백엔드 콜백 timeout·5xx와 네트워크 오류는 예외를 전파해 SQS가 다시 전달하게 한다.
- 게시물 처리 결과용 `upload-urls`가 404를 반환하면 업로드가 없거나 관리자가 이미 게시물을 삭제한
  영구 취소로 보고 원본·썸네일을 쓰지 않은 채 staging 객체를 삭제한다.
