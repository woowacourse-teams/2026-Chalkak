...
- 하네스 지침·스킬 변경 시 검사 사례 검토와 명시적으로 요청한 동작 평가에는 `$harness-evaluation`을 사용한다. 실제 AI 평가는 요청 없이 실행하지 않는다.
...
| 요청 | 적용 스킬 |
| --- | --- |
| 사용자가 직접 선택·호출한 심화 인터뷰 | `$chalkak-interview` (수동 전용) |
| 합의한 작업의 이슈·PR 단위, 의존 관계·진행 순서와 개발 중 재분할 | `$work-breakdown` |
| 합의한 분할안·이슈 초안으로 개발부터 PR까지 시작·재개 | `$development-workflow` |
| 이슈·PR 작성·등록 | `$issue-pr-workflow` |
| 중요한 기획·개발 선택의 합의 후 고민 기록 작성·갱신 | `$decision-notes` |
| 작업 브랜치 이름·생성·PR 대상·운영 릴리스 브랜치·작업 완료 후 정리 | `$branch-workflow` |
| 커밋 단위 설계·메시지 추천·커밋 생성·정리 | `$commit-conventions` |
| 백엔드 PR의 코드 리뷰·리뷰 댓글 작성 | `$backend-pr-review` |
...
- 이슈에 연결된 변경 작업의 착수·진행 갱신·재개에는 [작업 기록](.agents/skills/development-workflow/references/work-state.md)을 사용한다. 새 세션·단순 질문만으로 기록을 읽거나 만들지 않는다.
- 이슈·PR 양식은 `../.github/ISSUE_TEMPLATE/`와 `../.github/pull_request_template.md`에서 읽는다.
- 중요한 선택이 합의되면 `$decision-notes`으로 고민의 흐름을 기록하고 구현·검증 결과에 맞춰 갱신한다. 단순 대화에는 만들지 않으며 채팅·초안만 요청한 범위와 인터뷰 종료 요청을 우선한다.
...
| 작업 조건 | 적용 규칙·스킬 |
| --- | --- |
| `src/main/java/**/*.java` 작업 | `$main-code` |
| `src/test/java/**/*.java` 작업 | `$test-code` |
| `src/main/java`의 동작 추가·변경 또는 버그 수정 | `$tdd-workflow` |
| `src/main/java`의 클래스·패키지 생성·이동 또는 배치 위치 검토 | `$package-structure` |
| API JSON 필드·Parameter·URI, Java 변수·필드·매개변수·메서드, DB 식별자 또는 ErrorCode 이름 생성·변경·리뷰 | `$naming-conventions` |
| API Endpoint, Controller, Request·Response 계약 추가·변경·삭제 또는 API 버전 검토 | `$api-versioning` |
| `src/main/java`의 Request DTO 생성·수정·리뷰 | `$request-dto` |
| `src/main/java`의 날짜·시간 입력·조회·검증·변환·응답 처리 생성·수정·리뷰 | `$date-time` |
| 예외 정의·발생·변환 또는 예외 클래스·에러 코드·예외 응답·예외 처리기 변경 | `$exception-handling` |
| springdoc-openapi 설정, Swagger/OpenAPI 어노테이션, API 문서 인터페이스 또는 생성된 OpenAPI 계약 생성·수정·리뷰 | `$swagger-docs` |

- 운영 코드와 테스트 코드를 함께 변경하면 양쪽에 해당하는 규칙과 스킬을 모두 적용한다.
- 테스트 클래스·메서드와 패키지 이름에는 `$naming-conventions`를 사용하지 않는다.
- Swagger/OpenAPI 문서와 설정만 변경하는 작업에는 `$tdd-workflow`를 사용하지 않는다. 실제 API 동작도 변경하면 그 동작 변경에만 적용한다.
