...
| `RefreshTokenPolicy` | 비활동 만료·절대 만료·재사용 유예 시각 계산 |
...
회전 직후 10초는 재사용으로 판정하지 않는다. 액세스 토큰이 만료돼 401을 받은 클라이언트는 큐에
쌓인 요청을 재발급 뒤에 한꺼번에 다시 보내는데, 이때 같은 리프레시 토큰이 거의 동시에 두 번
도착하는 것이 정상 동작이다. 이 구간까지 탈취로 보면 멀쩡한 사용자의 세션이 끊긴다. 유예 안의
두 요청은 각자 후속 토큰을 받고 다음 회전에서 하나로 수렴한다.
