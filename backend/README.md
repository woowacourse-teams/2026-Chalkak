...
소셜 로그인과 회원가입 응답은 액세스 토큰과 리프레시 토큰을 함께 내준다. 액세스 토큰은 15분,
리프레시 토큰은 비활동 30일·절대 90일 동안 유효하다. 일반 API는 응답으로 받은 액세스 토큰을
`Authorization` 헤더에 담아 호출한다.
...
| `RefreshTokenService` | 리프레시 토큰 발급·회전·폐기. 회원과 관리자가 같은 절차를 공유 |
| `SecureRandomRefreshTokenProvider` | 리프레시 토큰 난수 발급과 저장용 SHA-256 해싱 |
| `RefreshTokenPolicy` | 비활동 만료·절대 만료 시각 계산 |
| `RefreshTokenCleanupScheduler` | 더는 쓰이지 않는 토큰 행을 매일 정리 |

공개 경로는 소셜 로그인·회원가입(`/api/v1/auth/**`), 토큰 재발급과 로그아웃
(`POST /api/v1/auth/refresh`, `POST /api/v1/auth/logout`,
`POST /api/v1/admin/auth/refresh`, `POST /api/v1/admin/auth/logout`), 주제와 게시물 조회(`GET`),
Lambda 콜백(`/internal/v1/**`, HMAC으로 자체 인증), 헬스체크다. 재발급과 로그아웃은 리프레시
토큰 자체가 자격증명이고 액세스 토큰이 만료된 뒤에 호출하므로 액세스 토큰을 요구하지 않는다.
이 목록은 `SecurityFilterChainTest`가 고정한다.
...
전달한다. 로그인·재발급·로그아웃만 공개이며 현재 관리자 조회와 나머지 관리자 API는 모두
`ADMIN` scope가 필요하다. 로그아웃은 제시한 리프레시 토큰이 속한 기기의 계보를 실제로 끊으므로
그 기기는 더 이상 재발급하지 못한다. 다만 이미 발급된 액세스 토큰은 서명만으로 검증되어
최대 15분 동안 살아 있다.
...
리프레시 토큰은 JWT가 아니라 불투명한 256비트 난수다. 토큰에 아무 의미도 담지 않으므로 서명
검증이 필요 없고, 서버는 SHA-256 해시만 저장해 DB가 유출돼도 저장된 값으로 토큰을 되돌릴 수
없다. 액세스 토큰과 달리 `Authorization` 헤더가 아니라 요청 본문으로 보낸다.
...
응답은 새 액세스 토큰과 함께 **새 리프레시 토큰**을 내준다. 재발급할 때마다 토큰을 회전시키므로
요청에 쓴 토큰은 그 자리에서 못 쓰게 되고, 클라이언트는 저장한 값을 응답의 것으로 교체해야 한다.
회전하지 않으면 한 번 새어 나간 토큰이 절대 만료까지 90일 내내 유효하다.

`session_id`는 기기 하나의 회전 계보를 가리킨다. 이미 회전된 토큰이 다시 들어오면 탈취로 보고
**그 계보만** 폐기한다. 다른 기기의 로그인을 그대로 두는 것은, 한 기기의 사고로 모든 기기를
로그아웃시키면 정상 사용자가 겪는 피해가 공격자가 얻는 것보다 커지기 때문이다.

재사용 판정에는 유예 구간이 없다. 대신 클라이언트가 재발급 호출을 직렬화한다. 앱 전체에서 재발급
요청은 한 번에 하나만 띄우고, 그 사이에 401을 받은 다른 호출은 새 요청을 만들지 않고 진행 중인
재발급의 결과를 기다렸다가 새 액세스 토큰으로 재시도한다. 회전된 토큰이 들어오면 그 값의 사본이
남아 있다는 뜻이므로 탈취로 보고 그 기기의 계보를 폐기한다.

다만 직렬화가 모든 재제시를 덮지는 못한다. 재발급 응답이 유실되면 클라이언트는 새 토큰을 받지
못한 채 같은 토큰으로 재시도할 수밖에 없고, 그때는 계보가 폐기되어 재로그인이 필요하다. 호출이
애초에 하나뿐이라 직렬화할 대상이 없는 경우다. 유예 구간을 두지 않은 대가이며, 틀리는 방향이
로그인 화면으로 보내는 쪽이라 탈취된 세션을 살려 두는 것보다 안전하다고 보고 받아들인다.

연동 후 강제 재로그인이 실제로 잦다면 유예 구간을 되살리는 대신 멱등 재생을 검토한다. 후속
토큰의 해시를 소비된 행에 함께 저장해 두고, 회전된 토큰이 다시 오면 새로 만들지 않고 저장해 둔
후속 토큰을 그대로 돌려주는 방식이다. 계보당 살아 있는 토큰이 하나라는 불변식을 지키면서 응답
유실을 견디고, 탐지 구간도 넓히지 않는다.

클라이언트 계약은 401 응답의 `errorCode`로 갈린다.

- `UNAUTHORIZED`: 액세스 토큰만 만료된 상태다. 재발급을 한 번 하고 원래 요청을 다시 보낸다.
- `REAUTHENTICATION_REQUIRED`: 재발급까지 실패했다. 저장한 토큰을 모두 버리고 로그인 화면으로
  보낸다. 다시 시도해도 결과가 달라지지 않는다.

탈퇴는 그 회원의 모든 계보를 폐기하지만 **정지는 폐기하지 않는다.** 정지 회원도 재발급 자체는
성공하고, 요청마다 `UsableUserPolicy`가 403으로 막는다. 정지는 되돌릴 수 있는 상태라 계보를
끊으면 해제할 때마다 모든 기기가 다시 로그인해야 하는데, 액세스 토큰만으로는 어차피 아무것도
통과하지 못하므로 그렇게까지 할 이유가 없다.

절대 만료가 지난 행과 폐기된 지 7일이 넘은 행은 매일 04:30 KST에 지운다. 회전됐지만 절대 만료
전인 행은 비활동 만료가 지났어도 남긴다. 이 행의 `rotated_at`이 재사용 탐지가 보는 유일한
증거라서, 지우면 탈취된 토큰이 아무 기록도 없는 새 토큰처럼 통과한다.
