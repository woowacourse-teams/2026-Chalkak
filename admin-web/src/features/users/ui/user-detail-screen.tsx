...
export function UserDetailScreen({ userId }: { userId: string }) {
  ...
  const returnTo = params.get("returnTo")?.startsWith("/users") ? params.get("returnTo")! : "/users";
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.detailHero}><div><p>USER PROFILE</p><h2>{user.email ?? "이메일 정보 없음"}</h2><span>{user.userId}</span></div><StatusBadge tone={display.tone}>{display.label}</StatusBadge></section>
    <div className={styles.detailGrid}>
      ...
      <section className={styles.panel}><h3>게시물 현황</h3><div className={styles.statGrid}>{Object.entries(user.postCounts).map(([key, value]) => <div key={key}><span>{({total:"전체",validating:"이미지 검증",pending:"검수 대기",approved:"승인",rejected:"거절"} as Record<string,string>)[key]}</span><strong>{value}</strong></div>)}</div></section>
    </div>
